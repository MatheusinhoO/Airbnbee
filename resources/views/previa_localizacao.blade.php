...
<body>
    <div class="container">
        ...
        <form action="{{ route('localizacao.salva')}}" method="POST">
        @csrf
            ...
            <div class="form-group">
                ...
                <textarea id="informacao_estacionamento" name="informacao_estacionamento" rows="3" placeholder="Ex: Vaga nº 10, estacionamento coberto, permitido 1 carro."></textarea>
            </div>

            <div class="form-group">
                <label for="calendario_disponibilidade">Calendario de Disponibilidade apartir de <span class="required">*</span></label>
                <input type="date" id="calendario_disponibilidade" name="calendario_disponibilidade" required>
            </div>

            <div class="form-group">
                <label for="duracao_estadia">Duração Mínima da Estadia (noites)</label>
                <input type="number" id="duracao_estadia" name="duracao_estadia" min="1" placeholder="Ex: 2">
            </div>

            <button type="submit" class="btn-submit">Cadastrar Localização</button>
        </form>
    ...
    <script>
        document.getElementById('cadastroLocalizacaoForm').addEventListener('submit', function(e) {
            e.preventDefault();
            
            const formData = {
                endereco: document.getElementById('endereco').value,
                entrega_chave_metodo: document.getElementById('entrega_chave_metodo').value,
                informacao_estacionamento: document.getElementById('informacao_estacionamento').value,
                calendario_disponibilidade: document.getElementById('calendario_disponibilidade').value,
                duracao_estadia: document.getElementById('duracao_estadia').value
            };
