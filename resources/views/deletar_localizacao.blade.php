...
<body>
    <div class="container">
        ...
        <form action="{{ route('apaga_localizacao')}}" method="POST">
        @csrf
            <div class="form-group">
                <label for="endereco">Endereço Completo </label>
                <input type="text" id="endereco" name="endereco" value="{{ $locacao->endereco }}"  disabled>
            </div>
           

            <div class="form-group">
                <label for="entrega_chave_metodo">Método de Entrega da Chave <span class="required">*</span></label>
                <select id="entrega_chave_metodo" name="entrega_chave_metodo" disabled>
            ...
            <div class="form-group">
                ...
                <textarea id="informacao_estacionamento" name="informacao_estacionamento" rows="3" placeholder="Ex: Vaga nº 10, estacionamento coberto, permitido 1 carro." disabled>{{ $locacao->informacao_estacionamento}}</textarea>
            </div>

            <div class="form-group">
                <label for="calendario_disponibilidade">Calendario de Disponibilidade apartir de <span class="required">*</span></label>
                <input type="date" id="calendario_disponibilidade" name="calendario_disponibilidade" disabled>{{ $locacao->calendario_disponibilidade}}
            </div>

            <div class="form-group">
                <label for="duracao_estadia">Duração Mínima da Estadia (noites)</label>
                <input type="number" id="duracao_estadia" name="duracao_estadia" min="1" placeholder="{{ $locacao->duracao_estadia}}" disabled>
            </div>

            <button type="submit" class="btn-submit">Deletar Localização</button>
        </form>
