    ...
    <style>
        * {
        ...
        body {
            ...
            background: linear-gradient(135deg, #e1ea66 0%, #f7ca04 100%);
            min-height: 100vh;
            display: flex;
...
<body>
    <div class="container">
        ...
        <p class="subtitle">Preencha os dados da sua propriedade</p>
        
        <form action="{{route('informacao.salva')}}" method="POST">
        @csrf
            ...
            <div class="form-group">
                ...
                <input type="number" id="capacidade_hospedes" name="capacidade_hospedes" min="1" max="50" required>
            </div>

            <div class="row">
                <div class="form-group">
                    <label for="quartos_quantidade">Quartos <span class="required">*</span></label>
                    <input type="number" id="quartos_quantidade" name="quartos_quantidade" min="0" max="20" required>
                </div>

                <div class="form-group">
                    <label for="camas_quantidade">Camas <span class="required">*</span></label>
                    <input type="number" id="camas_quantidade" name="camas_quantidade" min="1" max="50" required>
                </div>
            </div>

            <div class="form-group">
                <label for="banheiros_quantidade">Banheiros <span class="required">*</span></label>
                <input type="number" id="banheiros_quantidade" name="banheiros_quantidade" min="1" max="20" step="0.5" required>
            </div>

            <div class="form-group">
                <label for="preco_noite">Preço por Noite (R$) <span class="required">*</span></label>
                <input type="number" id="preco_noite" name="preco_noite" min="0" step="50" required>
            </div>
            ...
            <button type="submit" class="btn-submit">Cadastrar Locação</button>
        </form>
    ...
    <script>
        document.getElementById('cadastroForm').addEventListener('submit', function(e) {
            e.preventDefault();
            // Exibir dados no console (aqui você pode enviar para um backend)
            console.log('Dados da locação:', formData);
